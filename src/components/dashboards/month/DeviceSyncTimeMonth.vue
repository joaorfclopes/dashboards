...
export default {
  ...
  methods: {
    getData() {
      for (
        let i = 0;
        i < store.state.checkDeviceSyncTimeMonthData.length;
        i++
      ) {
        this.series[i] = {
          name: store.state.checkDeviceSyncTimeMonthData[i].title,
          data: store.state.checkDeviceSyncTimeMonthData[i].value
        };
      }
    }
  },
  ...
  created() {
    ...
    store.dispatch("fetchDeviceSyncTimeMonth").then(() => {
      this.loading = false;
    });
  }
};
