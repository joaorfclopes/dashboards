...
export default {
  ...
  methods: {
    getData() {
      for (let i = 0; i < store.state.checkSimStatusHourData.length; i++) {
        this.series[i] = store.state.checkSimStatusHourData[i].value;
        this.chartOptions.labels[i] =
          store.state.checkSimStatusHourData[i].title;
      }
    }
  },
  computed: {
