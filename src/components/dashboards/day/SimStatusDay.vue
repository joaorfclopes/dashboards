...
export default {
  ...
  methods: {
    getData() {
      for (let i = 0; i < store.state.checkSimStatusDayData.length; i++) {
        this.series[i] = store.state.checkSimStatusDayData[i].value;
        this.chartOptions.labels[i] =
          store.state.checkSimStatusDayData[i].title;
      }
    }
  },
  computed: {
